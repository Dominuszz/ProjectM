#include "Engine/EnemiesManager.h"
...
void EnemiesManager::spawnEnemy() {
    float texture_index=rand()%m_enemies_texture.size()-0;
    ...
    if((abs(start_x-m_player->getPos().x)>ENEMY_SPAWN_RANGE || abs(start_y-m_player->getPos().y)>ENEMY_SPAWN_RANGE)
        && !m_respawn_timer->isRunning()) {
        auto enemy = std::make_shared<Enemy>(m_enemies_texture[texture_index], sf::Vector2f(start_x, start_y),
                                             ENEMY_ATTACK_COOLDOWN, ENEMY_SPEED, ENEMY_DAMAGE,
                                             ENEMY_START_HP * m_enemies_hp_scale);
...
void EnemiesManager::addTexture(const sf::Texture &texture) {
    m_enemies_texture.push_back(texture);
}
